class Engine {
  ...
  updateEngineState( throttle, timestep ) {
    ...
    this._currentTorque = this._rot < 2 ? (throttle > 1.1 ? this._maximumHP : (this._rot < 0 ? 0 : -1000)) : 118.675 * this._currentPower / this._rot;
    // console.log(this._currentPower, this._currentTorque, this._rot);
    this._rot += this._currentTorque / (this._shaft_inertia + this._load_inertia) * timestep;// N * m / ( kg * m ^ 2 ) = N / (kg * m);
